...
### 6. Изображения не открываются на мобильных

**Проблема:**
Изображения статей не отображаются на мобильных устройствах, показываются пустые места.

**Причина:**
- Внешние изображения (Unsplash) блокируются на некоторых мобильных сетях
- Медленная загрузка на 3G/4G
- Нет обработки ошибок загрузки
- Проблемы с CORS или CSP на мобильных

**Решение:**

1. **Добавлена обработка ошибок в RecentNotes.tsx:**
```tsx
<img
  src={image}
  alt={title}
...
2. **Добавлены адаптивные стили для мобильных:**
```scss
...
3. **Создано руководство IMAGES_GUIDE.md** с решениями:
   - Миграция на локальные изображения
   - Оптимизация размеров и качества
   - Создание fallback изображений
   - Тестирование на мобильных

**Рекомендации:**
- Перейти на локальные изображения (см. docs/IMAGES_GUIDE.md)
- Оптимизировать размер изображений (< 100KB)
- Использовать WebP формат для лучшего сжатия
- Создать дефолтное изображение для статей без обложки

**Профилактика:**
- Всегда тестировать на реальных мобильных устройствах
- Использовать Chrome DevTools → Network Throttling (3G)
- Проверять размер изображений перед добавлением
- Иметь fallback для всех внешних ресурсов
